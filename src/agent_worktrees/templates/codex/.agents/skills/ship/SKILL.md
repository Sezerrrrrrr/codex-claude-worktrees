---
name: ship
description: Explicitly land the current persistent letter lane through a GitHub pull request, then retarget its local and remote letter branch to the merged default branch. Use only when the user invokes $ship or explicitly asks to land the lane on GitHub.
---
...
```bash
agent-worktrees ship --harness codex
```
